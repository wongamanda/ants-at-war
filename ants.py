...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        if self.exit is not None:
            self.exit.entrance, self = self, self.exit
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(self.ant, QueenAnt) and self.ant.true_queen: # do not remove the queen ant!
                return     
            if self.ant is insect:
                ...
                if isinstance(self.ant.ant, QueenAnt) and self.ant.ant.true_queen: # even if queen ant is in container!
                    return 
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if not self.place.ant or self.place.ant.blocks_path is False:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while nearest_place is None and other_place is not hive: # checks that the place is not a hive and nearest bee not yet found
            if forward >= self.min_range and forward <= self.max_range: # checks w/in range
                if other_place.bees:
                    nearest_place = other_place # saves place with bees
            other_place = other_place.entrance
            forward += 1
        if nearest_place is None:
            return None
        return random_or_none(nearest_place.bees) # returns random bee
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        for bee in self.place.bees[:]: # makes a copy of list to simultaneously mutate for
            bee.reduce_armor(self.damage)
...
class HungryAnt(Ant):
    ...
    food_cost = 4
    armor = 1
    time_to_digest = 3 # 3 turns to eat another bee
    ...
    def eat_bee(self, bee):
        if bee:
            ...
            self.digesting = self.time_to_digest # resets timer

    def action(self, colony):
        if self.digesting:
            self.digesting -= 1
        else:
            self.eat_bee(random_or_none(self.place.bees)) # eats a random bee
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        self.ant = ant # assigns non-container ant

    def action(self, colony):
        if self.ant is not None: # performs non-container ant action
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        for bee in self.place.bees[:]: # make a copy of list to mutate it
            bee.reduce_armor(self.damage)
        super().action(colony)
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        if QueenAnt.count_queens == 0: # creates true_queen
            self.true_queen = True # only the first queen is true
            QueenAnt.buff_ants.append(self)
    ...
    def action(self, colony):
        ...
        """
        other_place = self.place # does not mutate self.place

        if self.true_queen is False: # removes imposters
            Insect.reduce_armor(self, self.armor)
            return

        while other_place is not None:
            if other_place.ant is not None:
                if other_place.ant not in QueenAnt.buff_ants:
                    other_place.ant.damage = other_place.ant.damage * 2
                    QueenAnt.buff_ants.append(other_place.ant)
                if other_place.ant.container:
                    if other_place.ant.ant: # doubles non-container ant
                        if other_place.ant.ant not in QueenAnt.buff_ants:
                            other_place.ant.ant.damage = other_place.ant.ant.damage * 2
                            QueenAnt.buff_ants.append(other_place.ant.ant)
                    if other_place.ant not in QueenAnt.buff_ants: # doubles container ant
                        other_place.ant.damage *= 2
                        QueenAnt.buff_ants.append(other_place.ant) # appends to ensure doubling once     
            other_place = other_place.exit # moves backwards
